...
---
# ### Частина 6 
**Завдання: Динамічний фільтр для таблиць**

### Опис завдання
Вам потрібно реалізувати таблицю та динамічні фільтри для трьох сторінок. Фільтри мають відображатися залежно від конфігурації, що отримується з бекенду. Взаємодія з таблицею та фільтрами повинна включати можливості застосування та скидання фільтрів, з оновленням даних через запити до бекенду.

### Основні вимоги

1. **Таблиці**:
    - Кожна таблиця має свої дані, які повинні оновлюватися на основі фільтрів.

2. **Фільтри**:
    - На кожній із трьох сторінок має бути свій набір фільтрів.
    - Конфігурація фільтрів повинна приходити з бекенду.
    - Фільтри мають кнопку **Apply**, яка застосовує вибрані значення фільтрів до таблиці, та кнопку **Reset**, яка очищає фільтри та оновлює таблицю.

3. **Інтерфейси фільтрів**:
   Використовуйте наступні інтерфейси для типів фільтрів:
   ```typescript
...
4. **Додаткові умови**:
    - Динамічна генерація фільтрів: відображення повинно адаптуватися до конфігурації фільтрів з бекенду.
    - Зміна фільтрів на основі натискання кнопок **Apply** та **Reset** має оновлювати дані в таблиці через запит до бекенду.

### Додаткові підказки
- Реалізацію для відображення таблиці виконуйте тільки для однієї сторінки, а для решти забезпечте лише відображення фільтрів.
- Дизайн фільтрів може включати чекбокси, випадаючі списки та текстові поля залежно від типу фільтра (`checkbox`, `select`, `text`).


Запити для продуктів: Кожен магазин (Store A, Store B, Store C) має свій ендпоінт для отримання продуктів:
```
GET /api/storeA/products
GET /api/storeB/products
GET /api/storeC/products
```
```
GET /api/storeA/filters
GET /api/storeB/filters
GET /api/storeC/filters
```

# ### Частина 7

### Завдання: Створення фільтрів та управління станом для продуктів з використанням Store

#### Опис завдання:
1. **Фільтрація по сторінках**:
   - Для кожної сторінки створити окремий фільтр для вибору продуктів за різними критеріями.
   - Додати до Store стан фільтрів кожної сторінки, щоб зберігати та контролювати налаштування фільтрації.

2. **Управління станом**:
   - Усі дії з фільтрації, редагування та оновлення продуктів повинні виконуватись через Store.
   - Використовувати Store для зберігання стану фільтрів та поточного стану продуктів.

3. **Редагування продукту**:
   - Додати можливість редагування інформації про продукт через інтерфейс.
   - При зміні даних про продукт оновлювати його стан у Store.

4. **Оновлення даних з бекенду** (опціонально):
   - Якщо у вас є бекенд або сервіси для отримання даних, налаштувати автоматичне оновлення стану продуктів у Store при зміні даних на сервері.
   - Синхронізувати Store з бекендом при фільтрації та редагуванні продуктів.

#### Вимоги до виконання:
- Оформіть проект з урахуванням принципів чистого коду.
- Забезпечте зручність управління фільтрами на кожній сторінці.
- Реалізуйте коректне збереження стану у Store, щоб фільтри залишалися незмінними при поверненні на сторінки.
